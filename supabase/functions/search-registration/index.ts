...
function hasVsnpActivity(activities: string[] | null): boolean {
  ...
  return activities.some(activity =>
    activity
      .toLowerCase()
      .includes(VSNP_ACTIVITY.toLowerCase())
  );
}
/* =========================
   Server
...
serve(async (req) => {
  try {
    ...
    /* =========================
       Request body
    ========================= */
    const { mobile_number, action, context } = await req.json();
    ...
    ========================= */
    if (!mainParticipant) {
      const { data: accompParticipant } = await supabase
      ...
      if (accompParticipant) {
        const { data: mainForAccomp } = await supabase
          .from("varanasi_events_primary_participants")
          .select("full_name")
        ...
        return new Response(
          JSON.stringify({
            success: false,
            message: `You are registered as an accompanying participant for ${mainForAccomp.full_name}. Please contact them.`,
          }),
          {
            status: 200,
            headers: {
    ...
    if (action === "withdraw") {
      ...
      await supabase
        .from("varanasi_events_primary_participants")
        .update({ status: "false", withdrawal_date })
    ...
    /* =========================
       VSNP-only filtering (CONDITIONAL)
    ========================= */
    ...
    if (isVsnpContext) {
      filteredMain =
        hasVsnpActivity(mainParticipant.activities)
          ? mainParticipant
          : null;

      filteredAccomp =
        accompParticipants.filter(p =>
          hasVsnpActivity(p.activities)
        );

      if (!filteredMain && filteredAccomp.length === 0) {
        return new Response(
          JSON.stringify({
            success: false,
            message: "You are not registered for the Vishnu Sahasra Nama Parayanam event.",
          }),
          {
            status: 404,
            headers: {
